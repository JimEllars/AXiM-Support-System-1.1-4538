...
import { FiCpu, FiTerminal, FiX } from 'react-icons/fi';
  ...
  useEffect(() => {
    ...
    const startStream = async () => {
      try {
        ...
        if (!res.ok) throw new Error('Gateway rejected live investigation request.');
          ...
          for (const line of lines) {
            if (line.startsWith('data: ') && line !== 'data: [DONE]') {
              try {
                ...
                // Account for OpenAI/Deepseek json structure mutations
                const content = parsed.choices?.[0]?.delta?.content || parsed.content?.[0]?.text || '';
                if (content) {
                  setStreamText(prev => prev + content);
                }
              } catch (e) { /* catch half-formed fragments */ }
            }
          }
        }
      } catch (err) {
        setStreamText(prev => prev + '\n\n`[LIVE TRIAGE STREAM DISRUPTED]`');
      } finally {
  ...
  return (
    <div className="bg-zinc-950/80 border border-fuchsia-500/20 rounded-3xl p-6 mb-6 shadow-[0_0_30px_rgba(217,70,239,0.05)] relative overflow-hidden">
      ...
      <div className="flex justify-between items-center mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[9px] text-zinc-500 font-mono mt-0.5">Proxy Edge Pipeline Active</p>
          </div>
        </div>
      ...
      <div className="text-xs text-zinc-300 font-mono leading-relaxed bg-black/40 p-4 rounded-xl border border-zinc-900/50 max-h-[250px] overflow-y-auto prose-invert prose-p:my-1 prose-li:my-0.5">
        <ReactMarkdown>{streamText || 'Awaiting native chunk frames...'}</ReactMarkdown>
