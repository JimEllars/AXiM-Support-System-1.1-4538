...
export default function SupportMetrics() {
    ...
    const fetchMetrics = async () => {
      try {
        ...
        setMetrics({
          ...
          execResponses24h: execResponses || 0
        });
      } catch (err) {
  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 font-mono">
      ...
      {/* Exec Responses Velocity Card */}
      <div className="p-4 rounded-2xl bg-zinc-950/60 border border-zinc-800/80 backdrop-blur-md flex items-center justify-between">
        <div>
          <span className="text-[10px] text-zinc-500 font-bold uppercase tracking-wider">Exec Ingested (24h)</span>
          <div className="text-xl font-bold text-amber-400 mt-1">{metrics.execResponses24h}</div>
        </div>
